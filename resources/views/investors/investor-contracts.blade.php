    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Contract's List</h4>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <div id="example3_wrapper" class="dataTables_wrapper no-footer">
                        ...
                        <thead>
                            <tr role="row"><th class="sorting_asc" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-sort="ascending" aria-label=": activate to sort column descending" style="width: 27.9219px;">#</th><th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Name: activate to sort column ascending" style="width: 112.242px;">Investor Name</th> <th tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Project Name: activate to sort column ascending" style="width: 135.523px;">Project Name</th><th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Amount: activate to sort column ascending" style="width: 110.7344px;">Amount</th><th  tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Date: activate to sort column ascending" style="width: 136.062px;">Uploaded On:</th><th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Action: activate to sort column ascending" style="width: 51.4375px;">Action</th></tr>
                        </thead>
                        <tbody>
                            @foreach ($contracts as $contract)
                            <tr role="row" class="odd">
                                <td class="sorting_1">{{$loop->iteration}}</td>
                                <td><strong>{{$contract->investor->investor_name}}</strong></td>
                                <td>{{$contract->project->name}}</td>
                                <td>{{number_format($contract->amount)}}</td>
                                <td>{{$contract->created_at->format('d/m/Y')}}</td>
                                <td>
                                    <div class="action-buttons d-flex justify-content-center">
                                        <a href="{{route('user.investment-contracts-details',$contract->id)}}" class="mr-2 btn btn-secondary light">
